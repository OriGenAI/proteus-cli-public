...
def process_step(step, tmpdirname, source_url, bucket_url, cases_url):
    try:
        ...
        source_dir, _, output = func(path_name, path_name, inputs[0] if inputs else None, source_url, cases_url, **(additional_info or {}))

        # Post-Process the files
        if post_processing_function_name:
            post_func = getattr(preprocess_functions, post_processing_function_name)
            post_func(os.path.join(tmpdirname, "cases"), output, bucket_url, **post_processing_info)
