...
@click.option("--iterations", promp=config.PROMPT, default=config.STRESS_ITERATIONS)
@click.argument("bucket")
@click.argument("dataset_uuid")
@may_fail_on_http_error(exit_code=1)
@runs_authentified
def init_stress_test(bucket, dataset_uuid, workers=config.WORKERS_COUNT, iterations=config.STRESS_ITERATIONS):
    ...
    click.echo(init_keyword_check(bucket, dataset_uuid, workers=workers, iterations=iterations))
