...
@runs_authentified
def init_stress_test(bucket, file_ext, parallel_method="threads", workers=config.WORKERS_COUNT, iterations=config.STRESS_ITERATIONS):
    """This stress tests init file downloading and download integrity, simulating the preprocessing/training done by model runner"""
    from .init_keyword_check import keyword_check as init_keyword_check

    click.echo(init_keyword_check(bucket, file_ext, parallel_method, workers=workers, iterations=iterations))
